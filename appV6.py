...
class AlgorithmGUI:
    ...
    def _create_tab_lzw_fixed(self):
        key = 'lzw'
        ctrl_frame, widgets = self._create_base_tab(key, 'Kompresja LZW')
        self._add_file_selector(ctrl_frame, key, "Wybierz plik", [("Pliki Obrazów", "*.png *.jpg *.jpeg *.bmp"), ("Wszystkie pliki", "*.*")])
        self._add_process_button(ctrl_frame, key, row=1)
    ...
    def _process(self, key):
        ...
        widgets = self.widgets[key]
        path = self.paths[key]
        widgets['status_label'].config(text=f"Przetwarzanie {key.upper()}... Czekaj.")
        self.root.update_idletasks()

        results = None
        try:
            if key in ['rle', 'huffman', 'dct']:
                param = widgets['param_var'].get()
                if key == 'dct':
                    results = process_dct_and_get_results(path, param)
                elif key == 'rle':
                    results = process_rle_and_get_results(path, param)
                elif key == 'huffman':
                    results = process_huffman_and_get_results(path, param)
            elif key == 'dwt':
                results = process_dwt_and_get_results(path, widgets['wavelet_var'].get(), widgets['level_var'].get(), widgets['thresh_var'].get())
            elif key == 'lzw':
                results = process_lzw_and_get_results(path, simplification_level=1)
        except Exception as e:
        ...
        if orig is not None and not path.lower().endswith('.txt'):
            ...
            for panel in widgets['image_panels'].values():
